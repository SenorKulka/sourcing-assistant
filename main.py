...
def main():
    ...
    parser.add_argument("--get", type=str, metavar="URL", required=True, help="The 1688.com product URL to fetch information for.")
    parser.add_argument("--product", type=str, required=True, help="Product identifier or name (e.g., 'T-Shirt A') used for sheet organization.")
    parser.add_argument("--minmoq", type=int, help="Minimum MOQ to consider for pricing tiers. Filters tiers shown in the sheet.")
    parser.add_argument("--maxmoq", type=int, help="Maximum MOQ to consider for pricing tiers. Filters tiers shown in the sheet.")

    args = parser.parse_args()

    print(f"Sourcing Assistant: Processing URL: {args.get} for product: {args.product}")
    ...
    try:
        product_info = lovbuy.get_product_info_from_1688_url(args.get)

        if product_info:
            print(f"\nSuccessfully fetched product data from LovBuy for URL: {args.get}")
            ...
            print(f"Failed to retrieve product information from LovBuy for URL: {args.get}")
